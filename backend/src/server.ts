...
app.use(helmet())
app.use(cors({
  origin: ['http://localhost:3000','https://stackup.skartik.xyz','https://stackup-mu.vercel.app'], // Allow all origins for now - in production you might want to restrict this
  credentials: true,
...
const startServer = async () => {
  try {
    // Always try to connect to database first with retry logic
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(`❌ Database connection attempt ${attempt} failed:`, (dbError as Error).message)
    ...
    if (process.env.NODE_ENV !== 'production') {
      app.listen(PORT, () => {
        ...
        console.log(`🗄️  Database: ${dbConnected ? 'MongoDB' : 'In-Memory'}`)
